...
├── src/                        # 추론 소스
│   ├── inference.py            #   베이스라인 진입점: python -m src.inference
│   ├── model.py                #   출력 스키마 + vLLM 요청 구성
│   ├── utils.py                #   파싱/이미지 전처리
│   ├── phase1_unknown_heuristic.py  # unknown 옵션 인덱스 탐지 휴리스틱
│   ├── phase2_infer.py         #   본 추론기: BBQ 프롬프트/few-shot/SC/--system-v2
│   ├── build_dev_set.py        #   합성 dev셋 생성 (→ data/dev/dev.jsonl)
│   └── eval_dev.py             #   dev셋에서 프롬프트 변형 비교 채점
├── scripts/
...
├── models/                     # 모델 가중치/체크포인트
└── docs/                       # 문서
    ├── DESIGN_LOG.md           #   설계/실험 로그 (Phase 1~8)
...
- ⚠️ **과적합 주의**: public 점수만 보고 프롬프트/하이퍼파라미터를 반복 튜닝하면 private에서 무너질 수 있다.
  test 문항을 들여다보며 맞추는 방식보다 **일반 BBQ 추론 원리**(증거 기반 판단, 소거법, 고정관념 배제)로 푸는 쪽이 private 일반화에 안전하다. 미세 이득(예: ±0.002)은 노이즈일 수 있으므로 원리적 개선에 집중한다.
