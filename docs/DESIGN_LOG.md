...
- **진행:** 14B-v2 public 측정차 제출(`submissions/phase11_14b_textonly_v2.csv`, 포맷검증 통과).
- **다음 결정 실험:** 14B + v3(proxy·고정관념 비증거). 9B엔 v3 무용(고칠 편향 0)이었으나 14B는 실재 편향
  (+40%)이 있어 v3가 들을 수 있음. v3가 14B amb 편향을 낮추고 추론 보존하면 → 14B+v3 양 축 최강 후보.
